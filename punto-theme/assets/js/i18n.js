/* ============================================================
   ...
   Loads translations from JSON, persists choice, applies static
   [data-i18n] text and notifies subscribers so dynamic sections
   can re-render.

   NOTE (WordPress port): the JSON base path is read from
   window.PUNTO.dataBase when present (injected via
   wp_localize_script in functions.php). It falls back to the
   relative 'data/' path so the original static site keeps working.
   ============================================================ */
...
(function () {
  ...
  // Base URL for the language JSON files.
  var DATA_BASE = (window.PUNTO && window.PUNTO.dataBase) ? window.PUNTO.dataBase : 'data/';
  ...
  function loadTranslations() {
    return Promise.all(
      SUPPORTED.map(function (lang) {
        return fetch(DATA_BASE + lang + '.json')
          .then(function (res) {
